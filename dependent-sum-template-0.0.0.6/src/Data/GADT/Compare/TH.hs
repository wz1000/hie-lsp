...
instance DeriveGEQ Dec where
    ...
    deriveGEq (DataInstD dataCxt tyArgs' name _ cons _) = return <$> inst
#else
    deriveGEq (DataInstD dataCxt name tyArgs cons _) = return <$> inst
#endif
        where
            tyArgs = maybe [] (map go) tyArgs'
            go (PlainTV n) = VarT n
            go (KindedTV n _) = VarT n
            inst = instanceD (cxt (map return dataCxt)) (appT (conT ''GEq) (foldl1 appT (map return $ (name : init tyArgs)))) [geqDec]
...
instance DeriveGCompare Dec where
...
#if defined(__GLASGOW_HASKELL__) && __GLASGOW_HASKELL__ >= 612
#if __GLASGOW_HASKELL__ >= 800
    deriveGCompare (DataInstD dataCxt tyArgs' name _ cons _) = return <$> inst
#else
    deriveGCompare (DataInstD dataCxt name tyArgs cons _) = return <$> inst
        ...
        where
            ...
            go (PlainTV n) = VarT n
            go (KindedTV n _) = VarT n
            inst = instanceD (cxt (map return dataCxt)) (appT (conT ''GCompare) (foldl1 appT (map return $ (name : init tyArgs)))) [gcompareDec]
            -- TODO: figure out proper number of family parameters vs instance parameters
            bndrs = [PlainTV v | VarT v <- tail tyArgs ]
            gcompareDec = gcompareFunction bndrs cons
#endif
