...
instance DeriveGShow Dec where
    ...
    deriveGShow (DataInstD dataCxt tyArgs' name _ cons _) = return <$> inst
#else
    deriveGShow (DataInstD dataCxt name tyArgs cons _) = return <$> inst
        ...
        where
            ...
            inst = instanceD (cxt (map return dataCxt)) (appT (conT ''GShow) (foldl1 appT (map return $ (name : init tyArgs)))) [gshowDec]
