...
newtype EventM x a = EventM { unEventM :: IO a } deriving (Functor, Applicative, Monad, MonadIO, MonadFix, MonadException, MonadAsyncException, MonadFail) -- The environment should be Nothing if we are not in a frame, and Just if we are - in which case it is a list of assignments to be done after the frame is over
...
instance HasSpiderTimeline x => Semialign (Event x) where
  ...
  zip ea eb = mapMaybe go (align ea eb)
    where
...
instance HasSpiderTimeline x => Monad (Reflex.Class.Dynamic (SpiderTimeline x)) where
  ...
  {-# INLINE (>>) #-}
  (>>) = (*>)
instance HasSpiderTimeline x => MonadFail (Reflex.Class.Dynamic (SpiderTimeline x)) where
  {-# INLINE fail #-}
